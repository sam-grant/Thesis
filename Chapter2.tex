...
%This was basic principle used to indirectly measure $a_{\mu}$, prior to the CERN Muon $g-2$ experiments.

% \myworries{However, matters are complicated by the fact that the direction of the spin vector, and its rate of precession, are not directly observable. For the muon, which is both created and destroyed via the weak nuclear force, the rate of spin precession may instead be inferred from an understanding of parity, $P$, violation in weak decays.} 
...
%illustrating the $\omega_{a}$ oscillation resulting from an exponentially decaying population of muons, is shown 

% \clearpage

% Parity violation arguments (outlined in Section \ref{sec:ParityViolation}) give a correlation between $e^{+}$ emission angle and $\mu^{+}$ spin vector, so, provided that the muon beam is spin polarised on injection into the magnetic field, the population of high energy decay positrons will vary as a function of time with a frequency equal to $\omega_{a}$. 
%


% \begin{figure}[t!]
% \centering{}
% \includegraphics[trim={2cm 2cm 2cm 2cm},clip,width=\textwidth]{Images/Method/AnomalyFrequencyCartoon.pdf}
% \caption{A cartoon showing the relative precession of the spin and momentum vectors of a muon traversing a constant magnetic field, where the magnetic field lines are directed out of the page and the muons are injected at 12 o'clock. If $g_{\mu}=2$, then the two vectors will remain in phase; if $g_{\mu}\neq2$ then they will move out of phase. The rate of precession of the spin vector relative to the momentum vector is the anomaly frequency, $\vec{\omega}_{a}$, enabling a direct measurement of $a_{\mu}$.} 
% \label{fig:AnomalyFreq}
% \end{figure}

% \begin{figure}[t!]
% \centering{}
% \includegraphics[trim={0cm 0cm 0cm 0cm},clip,width=.69\textwidth]{Images/Method/ToyWiggle.pdf}
% \caption{A toy example of a population of high energy $e^{+}$ varying at the anomaly frequency, $\omega_{a}$.}
% \label{fig:ToyWiggle}
% \end{figure}
...
%
% \begin{equation}
%   \vec{\omega}_{c}=\frac{Qe}{m_{\mu}}\left[\frac{1}{\gamma}\vec{B}-\frac{\gamma}{\gamma^{2}-1}(\beta\times\vec{E})\right]
%   \label{eqn:FullCyclotron}
% \end{equation}
% %
% and 
% %
% \begin{equation}
%   \vec{\omega}_{s}=\frac{Qe}{m_{\mu}} \left[ \left(\frac{g_{\mu}}{2}-1+\frac{1}{\gamma}\right)\vec{B}-\left(\frac{g_{\mu}}{2}-1\right)\left(\frac{\gamma}{\gamma+1}\right)(\vec{\beta}\cdot\vec{B})\,\vec{\beta}-\left(\frac{g_{\mu}}{2}-\frac{\gamma}{1-\gamma}\right) (\vec{\beta} \times \vec{E}) \right], %(\vec{\beta}\times\vec{E}) 
%   \label{eqn:BMT}
% \end{equation}
% %
% where the modified expression for $\vec{\omega_{s}}$ is the Bargmann-Michel-Telegdi (BMT) equation \cite{BMT}. The resulting expression for $\vec{\omega}_{a}$, taking the difference between $\vec{\omega}_{c}$ and $\vec{\omega}_{s}$, is
%
\begin{equation}
...
% \myworries{People often like to write this out in a different way, such as below. I'm not sure what this adds, to me it seems to like an over-complication. Presumably there is a reason.} 

% \myworries{
% \begin{equation}
%   a_{\mu} = \frac{\omega_{a}/\omega_{p}}{\mu_{\mu}/\mu_{p}-\omega_{a}/\omega_{p}}=\frac{\mathcal{R}}{\lambda-\mathcal{R}}
% \end{equation}
% }

\section{Searching for a muon EDM}
% \section{Muon electric dipole moment search techique} %Searching for a muon electric dipole moment}
...
%
% \begin{equation}
%   P(\lambda, \theta, \phi, t) d\lambda d\Omega, \propto N(\lambda) (1+A(\lambda)\cos(\alpha(\theta, \phi, t)))
% \end{equation}
%
\begin{equation}
...
% \subsection{Sensitivity to a muon EDM Variation of the EDM vertical angle from relativistic effects}
...
%
% \begin{equation}
%   A_{\text{EDM}}(\lambda) \propto \frac{(\lambda-1)(2\lambda+1)}{4\lambda^{2}-5\lambda-5}\cdot \sin\delta^{*}.
%   \label{eqn:DilutionFunction}
% \end{equation} %\label{eqn:PriceFunction}
%
\begin{equation}
  \langle {\theta_{y}^{\text{EDM}}} \rangle (\lambda) \propto \frac{(\lambda-1)(2\lambda+1)}{4\lambda^{2}-5\lambda-5} \frac{\sin{\delta^{*}}}{\gamma},
  %\label{eqn:PriceFunction}
...
%
% \begin{equation}
%   d_{\text{EDM}}(\lambda) = \frac{1}{\delta}\cdot A_{\text{EDM}}(\lambda),
%   \label{eqn:Dilution}
% \end{equation} %\label{eqn:PriceFunction}
\begin{equation}
